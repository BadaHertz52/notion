...
const LinkLoader = ({
  ...
  const onChagneSearch = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === "") {
      ...
      if (isWebLink) {
        ...
        setWebLink(false);
        const candidateArray = notTemplatePages.filter((page: Page) =>
          page.header.title.includes(value)
        );
        candidateArray[0] !== undefined
  ...
  useEffect(() => {
    if (selectedHtml !== null) {
      ...
      if (selectedLink || parentLink || haveLinkElement) {
        ...
        const linkElementArray = [
          ...(selectedHtml.querySelectorAll(
            ".link"
          ) as NodeListOf<HTMLAnchorElement>),
        ];
        haveLinkElement && setLinkElements(linkElementArray);
      }
    }
  }, [selectedHtml]);
  type PageItemProps = {
  ...
  const PageItem = ({ page }: PageItemProps) => {
    ...
    return (
      <button className="page_inner" onClick={() => addLink(pagePath)}>
        <PageIcon
        ...
        <div className="pageInform">
          <div className="pageTitle">{page.header.title}</div>
          {page.parentsId !== null && (
        ...
        <div id="linkLoader_moreFn">
          ...
          {linked && (
            <div id="linkedFn">
              ...
              <button id="removeLinkBtn" onClick={removeLink}>
                <IoTrashOutline />
