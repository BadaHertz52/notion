...
export const CommentInput = ({
  ...
  const editComment = (event: MouseEvent) => {
    event.preventDefault();
    if (commentBlock !== null) {
      mainComment !== null ? editMainComment() : editSubComment();
    } else {
      mainComment !== null ? editPageComment() : editPageSubComment();
    }
    recoveryInputAfterSubmit();
  };
    ...
    if (block !== null) {
      ...
      if (comment !== null && block !== null && block.comments !== null) {
        ...
        const updateBlock = (changeContent: boolean) => {
          ...
          if (changeContent) {
            blockContents = document.getElementById(`${block.id}_contents`)
        ...
        if (mainCommentIds?.includes(comment.id)) {
          ...
          if (targetMainComment.selectedText !== null) {
            const textCommentBtnElement = document
              .getElementById(`${block.id}_contents`)
...
const CommentTool = ({
  ...
  const ResolveBtn = ({ comment }: ResolveBtnProps) => {
    ...
    return (
      <button className="resolveTool" onClick={changeToResolve}>
        <span>Resolve</span>
      </button>
    );
  };
  const openToolMore = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (
      ...
      if (blockCommentsHtml === null) {
        ...
        const pageComment =
          templateHtml == null
            ? (document.querySelector(".pageComment") as HTMLElement | null)
  ...
  return (
    <div className="commentTool" ref={commentToolRef}>
...
const CommentBlock = ({
  ...
  const blockContent = document.getElementById(
    `${block?.id}_contents`
  )?.textContent;
  useEffect(() => {
    // discard edit
    ...
    <div className="commentBlock">
      ...
      {mainComment && (
        <section className="comment_block">
          <div className="comment_block_line"></div>
          {block !== null && (
