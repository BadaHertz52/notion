...
export const getAccurateIndex = (
  ...
  if (children !== undefined) {
    ...
    if (node.parentNode?.nodeName !== "DIV") {
      const CHILD = findNodeInChilNodes(node, childrenArray);
      if (CHILD !== undefined) {
        childNode = CHILD;
      }
    }
    const nodeIndex = childrenArray.indexOf(childNode);
    const preNodes = childrenArray.slice(0, nodeIndex);
...
const BlockComponent = ({
  ...
  const [changedValue, setChangeValue] = useState<string>();
    ...
    if (page.blocks !== null && page.blocksId !== null) {
      ...
      const changeBlockContent = () => {
        if (value.includes("<div>")) {
          ...
          if (block.contents || offset || value) {
  ...
  const moveFocus = (nextBlockId: string, targetBlock: Block) => {
    ...
    if (contentsHtml !== null) {
      ...
      console.error(`Can't find .${targetBlock.id}_contents html`);
    }
  };
  /**
  ...
  return (
    ...
    >
      {block.type === "page" ? (
        <button className="contents pageTitle" id={`${block.id}_contents`}>
          <BlockContentEditable />
        </button>
      ) : block.type === "image" ? (
        block.contents === "" ? (
          <button className="addBlockFile" onClick={onClickAddFileBtn}>
            <span className="addBlockFileIcon">
