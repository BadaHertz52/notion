...
export  const findNodeInChilNodes=(node:Node ,childNodes:Node[])=>{
   ...
   */
export  const getAccurateIndex=(node:Node, block:Block ,contentEditableHtml:HTMLElement|null):{textIndex:number}=>{
    let totalSentence ="";
    const children = contentEditableHtml?.childNodes as NodeListOf<Node> | undefined;
   ...
   */ 
export const getFromStartNode=( startNode: Node, startOffset:number , block:Block ,contentEditableHtml:HTMLElement|null):{preChangedContent:string, selectedStartIndex:number}=>{
   ...
   */
export  const getFromEndNode=(endNode: Node,endOffset:number, block:Block ,contentEditableHtml:HTMLElement|null):{afterChangedContent:string, selectedEndIndex:number}=>{
    const contents =block.contents;
    /**
    ...
    const changeValueByFocus=(spanHtml:null|string)=>{
      ...
      if(contents.indexOf(text) === contents.lastIndexOf(text)){
        ...
        const textIndex= parentNode !==null?  getAccurateIndex(parentNode, block ,contentEditableHtml).textIndex :getAccurateIndex(endNode, block ,contentEditableHtml).textIndex ;
...
*/
export function updateStartParentNode(startOffset:number,startNode:Node,startNodeText:string, startParentNode:Node){
...
/**
  ...
  */
export function updateEndParentNode(endOffset:number,endNode:Node,endNodeText:string, endParentNode:Node){
...
/**
  ...
  */
export function updateMiddleChildren(startIndex:number, endIndex:number,endNode:Node, childNodes:Node[] , contentEditableHtml:HTMLElement|null){
...
export const selectContent =(SELECTION:Selection|null ,targetBlock:Block, contentEditableHtml:HTMLElement|null ,editBlock: ((pageId: string, block: Block) => void) | null,page:Page,setSelection:Dispatch<SetStateAction<selectionType|null>>|null)=>{
  ...
  if(SELECTION !==null && !notSelect ){
    ...
    if(anchorNode !==null && focusNode !==null && contentEditableHtml!==null){
      const contentEditableChild = contentEditableHtml.childNodes as NodeListOf<Node>;
      ...
      if(anchorNode.parentElement?.tagName !=="A" && focusNode.parentElement?.tagName !=="A"){
        ...
        if(startParentIsA&& endParentIsA &&(startNode=== endNode)){
          ...
          if(endOffset > startOffset+1){
            updateMiddleChildren(startIndex,endIndex,endNode,childNodes ,contentEditableHtml);
          };
