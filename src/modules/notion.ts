...
export const getBlockText = (block: Block) => {
  ...
  if (contentEditableHtml !== null && contentEditableHtml !== undefined) {
    ...
    children.forEach((c: Node) => {
      if (c.nodeType === 3) {
        c.nodeValue !== null && contentsArray.push(c.nodeValue);
      }
      if (c.nodeType === 1) {
        const element = c as HTMLElement;
        contentsArray.push(element.innerText);
      }
    });
    text = contentsArray.join("");
  }
  return text;
};
/**
...
export default function notion(
      ...
      case RAISE_BLOCK:
        /*  cursor.anchorOffset== 0 일때 backspace 를 누를때 단. targetPage의 fistBlocksId 의 첫번째 인수는 불가 
          result1 combine: previousBlockInDoc 의 content 수정 
            case1. action block 이 firstBlock 인 경우, 
  ...
  switch (action.type) {
    case ADD_PAGE:
      let page_array = pages !== null ? [...pages] : null;
      let pages_id = pagesId !== null ? [...pagesId] : null;
      let first_page_id = firstPagesId !== null ? [...firstPagesId] : null;

      if (pagesId !== null && pages !== null && firstPagesId !== null) {
        addPage(action.newPage);
      } else {
        page_array = [action.newPage];
        pages_id = [action.newPage.id];
        first_page_id = [action.newPage.id];
      }
      return {
        pages: pages !== null ? pages : page_array,
        firstPagesId: firstPagesId !== null ? firstPagesId : first_page_id,
        templatesId: templatesId,
        pagesId: pagesId !== null ? pagesId : pages_id,
        trash: trash,
      };
    ...
    case RESTORE_PAGE:
      ...
      page_array = pages !== null ? [...pages] : null;
      pages_id = pagesId !== null ? [...pagesId] : null;
      first_page_id = firstPagesId !== null ? [...firstPagesId] : null;
      ...
      if (page_array !== null && pages_id !== null && first_page_id !== null) {
        page_array?.push(restoredPage);
        pages_id.push(restoredPage.id);
        first_page_id.push(restoredPage.id);
      } else {
        page_array = [restoredPage];
        pages_id = [restoredPage.id];
        first_page_id = [restoredPage.id];
      }
